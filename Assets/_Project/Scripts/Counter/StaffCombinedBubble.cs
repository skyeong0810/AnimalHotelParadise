...
namespace AnimalHotel.Counter
{
    public class StaffCombinedBubble : MonoBehaviour
    {
        ...
        // === 대사 + 선택지 ===
        public IEnumerator ShowLineWithChoices(string lineText, List<string> options)
        {
            ClearButtons();
            ...
            if (options != null && options.Count > 0)
            {
                ...
                for (int i = 0; i < options.Count; i++)
                {
                    ...
                    _spawnedButtons.Add(CreateButton(options[i], i, y));
                }
            }
        ...
        private GameObject CreateButton(string text, int index, float localY)
        {
            ...
            btn.Setup(index, sr, buttonNormalColor, buttonHoverColor, OnButtonClicked);
            return go;
        }
